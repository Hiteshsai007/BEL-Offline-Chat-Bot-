...
def main() -> None:
    ...
    with console.status(
        "[bold yellow]Initializing offline AI core & loading FAISS index...[/bold yellow]",
        spinner="dots"
    ):
        try:
            get_retriever()
    ...
    while True:
        try:
            ...
            with console.status(
                "[bold magenta]Retrieving knowledge & generating response...[/bold magenta]",
                spinner="bouncingBar"
            ):
            ...
            if result.retrieved_chunks:
                table = Table(
                    title="[dim]Retrieved Context (Traceability)[/dim]",
                    show_header=True,
            ...
            if result.guardrail_triggered:
                console.print(
                    ...
                    "Response was regenerated due to citation hallucination guardrail."
                )
